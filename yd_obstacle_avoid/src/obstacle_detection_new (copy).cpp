...
bool obstacle_detection_new::detection(float &obs_dis)
{
    ...
    Vector3 p0(robot_pose.pose.position.x, robot_pose.pose.position.y, 0);
    ROS_DEBUG_STREAM("start x:" << p1.x << " y:" << p1.y);
    ROS_DEBUG_STREAM("end x:" << p2.x << " y:" << p2.y);
    ROS_DEBUG_STREAM("p0 x:" << p0.x << " y:" << p0.y);
    float p1x = p0.x - p1.x;
    float p1y = p0.y - p1.y;
    float p2x = p2.x - p1.x;
    float p2y = p2.y - p1.y;
    ...
    ROS_DEBUG_STREAM("nor x:" << nor.x << " y:" << nor.y);
    Vector3 proje_pos(p1.x + projection * nor.normalized().x, p1.y + projection * nor.normalized().y, 0);
    ROS_DEBUG_STREAM("proje_pos x:" << proje_pos.x << " y:" << proje_pos.y);
    //从投影点向终点延伸一定距离(radiu),判断是否有障碍物
    //先确定到终点的距离和检测距离的大小
    float final_length = remdis > detection_length ? detection_length : remdis;
    bool is_obs = false;
    // calc left down
    Vector3 right_v(nor.normalized().vertical().x, nor.normalized().vertical().y, 0);
    Vector3 temp_pos(0, 0, 0);
    temp_pos.x = proje_pos.x - right_v.x * robot_width / 2;
    temp_pos.y = proje_pos.y - right_v.y * robot_width / 2;
    temp_pos.z = proje_pos.z - right_v.z * robot_width / 2;
    ...
    {
        double cell_pox_x = temp_pos.x + right_v.x * resolution_ * i;
        double cell_pox_y = temp_pos.y + right_v.y * resolution_ * i;
        ...
        {
            double cell_pox_x_ = cell_pox_x + nor.normalized().x * resolution_ * j;
            double cell_pox_y_ = cell_pox_y + nor.normalized().y * resolution_ * j;
