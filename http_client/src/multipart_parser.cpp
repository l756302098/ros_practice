...
const std::string &MultipartParser::GenBodyContent()
{
  ...
  for(auto &file:files_)
  {
    ...
    {
      std::cout << "file name :" << file.second << std::endl;
      //custom
      // std::ifstream ifile(file.second, std::ifstream::binary);
      // std::vector<char> data( ( std::istreambuf_iterator<char>( ifile ) ), std::istreambuf_iterator<char>() );
      // std::string code = base64_encode((unsigned char*)&data[0], (unsigned int)data.size(),false);
      // ifile.close();
      // return code;
