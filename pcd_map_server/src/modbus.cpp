 ...
 */
void modbus::modbus_read_coils(int address, int amount, int& buffer) {
    if(_connected) {
 ...
 */
void modbus::modbus_read_input_bits(int address, int amount, int& buffer) {
    if(_connected) {
        if(amount > 2040 || address > 65535) {
            throw modbus_amount_exception();
        }
